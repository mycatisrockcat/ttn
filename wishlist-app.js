...
function getAllTagsInCategory(){
  const tagSet = new Set();
  (items[activeCategory]||[]).forEach(item => {
    if (item.tag) {
      item.tag.split(',').map(t=>t.trim()).filter(Boolean).forEach(t=>tagSet.add(t));
    }
  });
  return [...tagSet].sort();
}
...
function renderTagFilterBar(){
  ...
  const hasTagField = CATEGORIES[activeCategory].fields.includes('tag');
  if (!hasTagField) { bar.style.display='none'; bar.innerHTML=''; return; }

  const tags = getAllTagsInCategory();
  ...
  allChip.addEventListener('click', () => { activeTagFilter=null; renderTagFilterBar(); renderItems(); });
  ...
  tags.forEach(tag => {
    ...
    chip.addEventListener('click', () => { activeTagFilter = (activeTagFilter===tag?null:tag); renderTagFilterBar(); renderItems(); });
...
function renderItems(){
  ...
  grid.innerHTML = '';
  let list = items[activeCategory] || [];

  if (activeTagFilter) {
    list = list.filter(item => item.tag && item.tag.split(',').map(t=>t.trim()).includes(activeTagFilter));
  }
