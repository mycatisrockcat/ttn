...
async function openTrip(id){
  ...
  try {
    ...
    if (file) { currentTripSha = file.sha; currentTrip = decodeContent(file); }
      ...
      currentTrip = { id, overview: { name: meta.name||'', destination:'', start:meta.start||'', end:meta.end||'', companions:'', budget:'', status:meta.status||'예정', cover:meta.cover||'', memo:'' }, schedule: [], expenses: [], checklist: { packing:[], booking:[], before:[] }, records: {} };
    }
    setSyncStatus('ok','동기화됨');
...
function renderRecordDateBar(){
  ...
  dates.forEach(date => {
    ...
    chip.addEventListener('click', () => { activeRecordDate = date; renderRecordDateBar(); renderRecordContent(); });
...
function renderRecordContent(){
  ...
  if (!currentTrip.records[activeRecordDate]) currentTrip.records[activeRecordDate] = { photos: [], diary: '' };
  ...
  // 일기
  document.getElementById('record-diary-input').value = record.diary || '';
}

function renderRecordGallery(){
  const record = currentTrip.records[activeRecordDate];
  const grid = document.getElementById('record-gallery-grid');
  grid.innerHTML = '';
  (record.photos||[]).forEach((p, i) => {
    const card = document.createElement('div');
  ...
  grid.querySelectorAll('.gallery-del').forEach(btn => btn.addEventListener('click', async e => {
    record.photos.splice(+e.currentTarget.dataset.i, 1);
    renderRecordGallery();
    await saveCurrentTrip();
  }));
}
...
document.getElementById('record-file-input').addEventListener('change', e => {
  ...
  files.forEach(f => {
    ...
    r.onload = async ev => {
      record.photos.push({ image: ev.target.result });
      done++;
      if (done === files.length) { renderRecordGallery(); await saveCurrentTrip(); }
    };
    r.readAsDataURL(f);
  });
});
...
document.getElementById('record-diary-save-btn').addEventListener('click', async () => {
  if (!activeRecordDate) return;
  currentTrip.records[activeRecordDate].diary = document.getElementById('record-diary-input').value;
  await saveCurrentTrip();
  showToast('일기가 저장됐어요!');
});
